...
class Question extends CI_Controller {
    ...
    public function getAllQuestionsNew() {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            $admin_id = $this->input->get('admin_id');
            $data = json_decode(file_get_contents("php://input"));
            $limit = $data->limit;
            $start = $data->start;
            $questions = $this->Question_model->getQuestionsLimitNew($limit, $start);
            $questionNew_total = $this->Question_model->getQuestionTotalNew();
            $questionIgnoreEtc_total = $this->Question_model->getQuestionTotalIgnoredEtc();
            $questionFaq_total = $this->Question_model->getQuestionTotalFaq();
            $questionUploaded_total = $this->Question_model->getQuestionTotalUploaded();
            $questionAnswered_total = $this->Question_model->getQuestionTotalAnswered();
            $questionAssigned_total = $this->Question_model->getQuestionTotalAssigned();
            //Getting all FAQ from table
            $faq = $this->Question_model->getAllFaq();
        }
        ...
        echo json_encode(array("status" => "success", "questions" => $questions, 
            "questionNew_total" => $questionNew_total['new_total'], "questionIgnoreEtc_total" => $questionIgnoreEtc_total['ignoredEtc_total'],
    ...
    public function getAllQuestionsAssigned() {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            $admin_id = $this->input->get('admin_id');
            $data = json_decode(file_get_contents("php://input"));
            $limit = $data->limit;
            $start = $data->start;
            $questions = $this->Question_model->getQuestionsLimitAssigned($limit, $start);
           $questionNew_total = $this->Question_model->getQuestionTotalNew();
            $questionIgnoreEtc_total = $this->Question_model->getQuestionTotalIgnoredEtc();
            $questionFaq_total = $this->Question_model->getQuestionTotalFaq();
            $questionUploaded_total = $this->Question_model->getQuestionTotalUploaded();
        ...
        echo json_encode(array("status" => "success", "questions" => $questions, 
            "questionNew_total" => $questionNew_total['new_total'], "questionIgnoreEtc_total" => $questionIgnoreEtc_total['ignoredEtc_total'],
            "questionFaq_total" => $questionFaq_total['faq_total'], "questionUploaded_total" => $questionUploaded_total['uploaded_total'],
            "questionAnswered_total" => $questionAnswered_total['answered_total'], "questionAssigned_total" => $questionAssigned_total['assigned_total']));
        return;
    ...
    public function getAllQuestionsAnswered() {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $limit = $data->limit;
            $start = $data->start;
            $questions = $this->Question_model->getQuestionsLimitAnswered($limit, $start);
            $questionNew_total = $this->Question_model->getQuestionTotalNew();
            $questionIgnoreEtc_total = $this->Question_model->getQuestionTotalIgnoredEtc();
            $questionFaq_total = $this->Question_model->getQuestionTotalFaq();
            $questionUploaded_total = $this->Question_model->getQuestionTotalUploaded();
    ...
    public function getAllQuestionsUploaded() {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            $admin_id = $this->input->get('admin_id');
            $data = json_decode(file_get_contents("php://input"));
            $limit = $data->limit;
            $start = $data->start;
            $questions = $this->Question_model->getQuestionsLimitUploaded($limit, $start);
            $uploadedTime = $this->Question_model->getQuestionsUploadedTime($limit, $start);
            $questionNew_total = $this->Question_model->getQuestionTotalNew();
            $questionIgnoreEtc_total = $this->Question_model->getQuestionTotalIgnoredEtc();
            $questionFaq_total = $this->Question_model->getQuestionTotalFaq();
            $questionUploaded_total = $this->Question_model->getQuestionTotalUploaded();
        ...
        echo json_encode(array("status" => "success", "questions" => $questions, 
            "questionNew_total" => $questionNew_total['new_total'], "questionIgnoreEtc_total" => $questionIgnoreEtc_total['ignoredEtc_total'],
            "questionFaq_total" => $questionFaq_total['faq_total'], "questionUploaded_total" => $questionUploaded_total['uploaded_total'],
            "questionAnswered_total" => $questionAnswered_total['answered_total'],
            "questionAssigned_total" => $questionAssigned_total['assigned_total'], "uploadedTime" => $uploadedTime));
        return;
        
    }

    public function getAllQuestionsFaq() {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            $admin_id = $this->input->get('admin_id');
            $data = json_decode(file_get_contents("php://input"));
            $limit = $data->limit;
            $start = $data->start;
            $questions = $this->Question_model->getQuestionsLimitFaq($limit, $start);
            $totalFaqAssigned = $this->Question_model->getTotalFaqAssigned();
            $questionNew_total = $this->Question_model->getQuestionTotalNew();
            $questionIgnoreEtc_total = $this->Question_model->getQuestionTotalIgnoredEtc();
            $questionFaq_total = $this->Question_model->getQuestionTotalFaq();
            $questionUploaded_total = $this->Question_model->getQuestionTotalUploaded();
        ...
        echo json_encode(array("status" => "success", "questions" => $questions, 
            ...
            "questionFaq_total" => $questionFaq_total['faq_total'], "questionUploaded_total" => $questionUploaded_total['uploaded_total'],
            "questionAnswered_total" => $questionAnswered_total['answered_total'],
            "questionAssigned_total" => $questionAssigned_total['assigned_total'], "totalFaqAssigned"=>$totalFaqAssigned));
        return;
        
    }

    public function getAllQuestionsIgnoredEtc() {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            $admin_id = $this->input->get('admin_id');
            $data = json_decode(file_get_contents("php://input"));
            $limit = $data->limit;
            $start = $data->start;
            $questions = $this->Question_model->getQuestionsLimitIgnoredEtc($limit, $start);
            $questionNew_total = $this->Question_model->getQuestionTotalNew();
            $questionIgnoreEtc_total = $this->Question_model->getQuestionTotalIgnoredEtc();
            $questionFaq_total = $this->Question_model->getQuestionTotalFaq();
            $questionUploaded_total = $this->Question_model->getQuestionTotalUploaded();
 ...
 public function updateQuestionStatus() {
        if ($this->input->server('REQUEST_METHOD') == 'GET') {
            ...
            }elseif ($question_action == "Only Urdu") {
                $question_action_id = 8;
            }elseif ($question_action == "Hang Up") {
                $question_action_id = 9;
            }elseif ($question_action == "Silent") {
                $question_action_id = 10;
            }elseif ($question_action == "Misplaced") {
                $question_action_id = 11;
            ...
            if ($this->Question_model->updateQuestion($question_id, $question_update_array)) {
                ...
                if ($question_action_id != 1) {
                    ...
                    if ($question_action_id == 2) {
                        ...
                        $result = curl_exec($ch);
                        curl_close($ch);
                    } else if ($question_action_id == 8) {
                        $url = "http://127.0.0.1/MVP/Scripts/createAnswerRequest.php?question_id=$question_id&answer_id=-1&type=OnlyUrdu";
                        $ch = curl_init();
                        curl_setopt($ch, CURLOPT_URL, $url);
                        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                        curl_setopt($ch, CURLOPT_TIMEOUT, 20);
